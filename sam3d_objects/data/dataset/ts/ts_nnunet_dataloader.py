    ...
    def transform(tensor: torch.Tensor) -> torch.Tensor:
        ...
        if blur_iterations > 0:
            ...
            for _ in range(blur_iterations):
                out = F.avg_pool3d(out, kernel_size=kernel, stride=1, padding=padding, count_include_pad=False)
            if needs_batch:
                out = out.squeeze(0)
...
def build_domain_shift_transform(cfg: Optional[Dict]) -> Optional[tio.transforms.Transform]:
...
def GetTestTransforms(target_shape, domain_shift_cfg: Optional[Dict] = None):
...
class TS_nnUNet_Dataset(Dataset):
    def __init__(
        ...
        domain_shift_config: Optional[Dict] = None,
    ):
        ...
        self.num_categorical_classes = self.num_foreground_classes + (1 if self.categorical_use_background else 0)
        ...
        if isinstance(self.domain_shift_config, dict):
            ...
            self._apply_domain_shift = bool(self.domain_shift_config.get('enabled', False)) and self.mode.lower() in apply_modes
        else:
            self.domain_shift_config = {}
        
        print(f"DEBUG: TS_nnUNet_Dataset mode={self.mode}, _apply_domain_shift={self._apply_domain_shift}, domain_shift_config={self.domain_shift_config}")
    ...
    def get_file_names(self):
        ...
        if missing_cases:
            raise RuntimeError(
                f"Split references unknown cases: {missing_cases[:5]}"
            )
