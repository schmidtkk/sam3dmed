...
def get_spacing_for_modality(modality: str, override: Optional[float] = None) -> Tuple[float, float, float]:
...
def extract_and_cache_slices(
    ...
    for axis in axes:
        ...
        for idx in range(depth):
            ...
            xyz_masked = np.where(mask_2d[..., None] > 0, world_coords, np.nan).astype(np.float32).transpose(2, 0, 1)
...
def extract_mesh(mask: np.ndarray, affine: np.ndarray, out_path: Path, class_id: int = 1, level: float = 0.5) -> bool:
...
def main():
    ...
    parser.add_argument('--original_nifti_dir', required=True, help="Input directory with raw NIfTI files")
    ...
    parser.add_argument('--spacing', type=float, default=None, help="Target isotropic spacing (mm). If None, uses adaptive per-modality defaults")
    parser.add_argument('--modality', type=str, default=None, choices=['ct', 'mri', 'ultrasound'], help="Force modality (otherwise auto-detect)")
    parser.add_argument('--axes', default='0,1,2', help="Comma-separated list of axes to slice along")
    parser.add_argument('--extract_mesh', action='store_true', help="Extract meshes using marching cubes")
    parser.add_argument('--min_foreground_ratio', type=float, default=0.01, help="Skip slices with less foreground than this ratio")
    parser.add_argument('--overwrite', action='store_true', help="Overwrite existing outputs")
    parser.add_argument('--save_metadata', action='store_true', help="Save processing metadata as JSON")
    ...
    if len(imgs) == 0:
        imgs = [p for p in src_dir.glob("**/*.nii*") if not p.name.lower().endswith(("_seg.nii.gz", "_mask.nii.gz"))]
    ...
    for img_path in imgs:
        # try to find mask
        mask_candidates = (
            list(img_path.parent.glob(img_path.stem + '*seg*.nii*')) +
            list(img_path.parent.glob(img_path.stem + '*mask*.nii*'))
        )
        ...
        if img_data.ndim == 4:
            print(f"  4D image detected, using first volume")
            img_data = img_data[..., 0]
        ...
        try:
            subject = tio.Subject(image=tio.ScalarImage(tensor=img_data[np.newaxis], affine=img_nii.affine),
                                  mask=tio.LabelMap(tensor=mask_data[np.newaxis].astype(np.int32), affine=mask_nii.affine))
            resample = tio.Resample(target_spacing)
        ...
        num_slices = extract_and_cache_slices(
            img_np, mask_np, affine, case_id, dst_dir,
            axes=axes, sdf_path=sdf_out_path,
