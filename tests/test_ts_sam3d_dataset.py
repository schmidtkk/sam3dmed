...
def test_slice_filtering_logic():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert len(filtered_low) == 3, f"Expected 3 slices with threshold 0.01, got {len(filtered_low)}"
        
        # High threshold
        filtered_high = filter_by_occupancy(all_files, 0.3)
        assert len(filtered_high) == 1, f"Expected 1 slice with threshold 0.3, got {len(filtered_high)}"
